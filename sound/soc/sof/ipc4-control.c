...
static int sof_ipc4_set_bytes_data(struct snd_sof_dev *sdev,
				   struct snd_sof_control *scontrol, bool lock)
{
	struct sof_ipc4_control_data *cdata = scontrol->ipc_control_data;
	...
	/* Send the new data to the firmware */
	if (!pm_runtime_active(sdev->dev))
		return 0;

	msg->extension = SOF_IPC4_MOD_EXT_MSG_PARAM_ID(data->ipc4.param_id);
	...
	ret = sof_ipc4_set_get_kcontrol_data(scontrol, true, lock);
	if (ret < 0)
		dev_err(sdev->dev, "Failed to set bytes update for %s\n",
			scontrol->name);
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	sof_ipc4_set_bytes_data(sdev, scontrol, true);
				  ...
				  unsigned int size, bool from_dsp)
{
	...
	if (from_dsp) {
		struct snd_sof_dev *sdev = snd_soc_component_get_drvdata(scomp);
		int ret = sof_ipc4_set_bytes_data(sdev, scontrol, true);
	...
	list_for_each_entry(scontrol, &sdev->kcontrol_list, list) {
		if (scontrol->comp_id == swidget->comp_id) {
			...
			case SND_SOC_TPLG_CTL_BYTES:
				ret = sof_ipc4_set_bytes_data(sdev, scontrol,
							      false);
				break;
			default:
				break;
